...
class BaseModelTorch(BaseModel):
    ...
    def fit(self, X, y, X_val=None, y_val=None, r_val=0.1, time_limit=600):
        ...
        for epoch in range(self.epochs):
            ...
            self.check_any_saved_model_exists()
    ...
    def save_model(self, filename_extension="", directory="models"):
        ...
        assert os.path.exists(filename), f"ALERT! failed to save model to {filename}"
        print(f"saved model to {filename}!")

    def check_any_saved_model_exists(self, filename_extension="best"):
        filename = get_output_path(
            os.path.join(self.__class__.__name__, self.run_id),
        ...
        assert os.path.exists(filename), f"ALERT! saved model missing! from {filename}"
